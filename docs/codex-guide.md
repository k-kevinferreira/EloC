...
- CRUD administrativo do catalogo implementado no frontend para:
  ...
  - `subcategories`
  - `products`
- modelagem relacional inicial de imagens de produto implementada com `ProductImage`
...
- `npm run prisma:validate --workspace @eloc/backend` executado com sucesso
- `npm run prisma:generate --workspace @eloc/backend` executado com sucesso
- `npm run prisma:migrate:deploy --workspace @eloc/backend` executado com sucesso
- `npm run build:backend` executado com sucesso
- `npm run lint --workspace @eloc/frontend` executado com sucesso
- `npm run build --workspace @eloc/frontend` executado com sucesso
...
Na ultima etapa relevante, foi concluida a primeira evolucao estrutural de imagens de produto sobre a base administrativa do catalogo.

Entregas realizadas:

- criacao da entidade relacional `ProductImage` no Prisma
- criacao e aplicacao da migration `20260423_add_product_images`
- sincronizacao de compatibilidade entre `images[]` e `Product.imageUrl` no backend
- ajuste dos DTOs e do service de `products` para tratar `images[]` como contrato principal
- ajuste do painel administrativo de `products` para enviar galeria por URL manual com imagem principal, `altText` e `displayOrder`
- validacao completa de Prisma, build do backend, lint do frontend e build do frontend
...
- upload deve continuar tratado como problema de infraestrutura; o storage local atual e uma base inicial e deve poder ser trocado por provider externo sem alterar `ProductImage`
...
- painel administrativo de produtos em `apps/frontend/src/app/(admin)/admin/(protected)/products` e `apps/frontend/src/components/admin/product-management.tsx`
